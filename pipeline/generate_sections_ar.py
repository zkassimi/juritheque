...
def list_missing(only_with_fr=False):
    content = SEO_FILE.read_text(encoding='utf-8')
    slugs = re.findall(r"slug:\s*['\"]([^'\"]+)['\"]", content)
    missing = []
    for slug in slugs:
        g = read_guide(slug)
        if g and not g['has_sections_ar']:
...
def sections_ar_to_js(sections: list) -> str:
    ...
    for s in sections:
        lines.append('    {')
        lines.append(f"      h2: '{s['h2'].replace(chr(39), chr(92)+chr(39))}',")
        lines.append(f"      content: '{s.get('content','').replace(chr(39), chr(92)+chr(39))}',")
        lines.append('      bullets: [')
        for b in s.get('bullets', []):
            lines.append(f"        '{b.replace(chr(39), chr(92)+chr(39))}',")
        lines.append('      ],')
        lines.append('    },')
    lines.append('  ],')
    return '\n'.join(lines)
...
def patch_file(slug: str, sections_ar: list, dry_run: bool) -> bool:
    content = SEO_FILE.read_text(encoding='utf-8')
    m = re.search(rf"slug:\s*['\"]({re.escape(slug)})['\"]", content)
    if not m:
    ...
    js_code = '\n' + sections_ar_to_js(sections_ar) + '\n'

    new_content = content[:insert_pos] + js_code + content[insert_pos:]

    if dry_run:
        print(f'  [dry-run] sections_ar générées ({len(sections_ar)} sections) — non écrites')
        # Afficher un aperçu
        for s in sections_ar[:2]:
            print(f"    h2: {s['h2'][:60]}")
            for b in s.get('bullets', [])[:2]:
                print(f"      • {b[:70]}")
        return True

    SEO_FILE.write_text(new_content, encoding='utf-8')
    print(f'  ✓ sections_ar insérées dans seoIntentPages.js')
    return True
...
def process_guide(slug: str, dry_run: bool) -> bool:
    print(f'\n🔷 {slug}')
    guide = read_guide(slug)
    if not guide:
        print('  ✗ guide introuvable')
        return False
    if guide['has_sections_ar']:
        print('  ✓ sections_ar déjà présentes, skip')
        return True

    mode = 'traduction FR→AR' if guide['sections_fr'] else 'génération depuis 0'
    print(f'  → {mode} ({len(guide["sections_fr"])} sections FR)')
    ...
    print(f'  ✓ {len(sections)} sections générées')
    return patch_file(slug, sections, dry_run)
...
def main():
    ...
    if args.slug:
        process_guide(args.slug, args.dry_run)
    elif args.all:
        guides = list_missing(only_with_fr=args.with_fr_only)
        print(f'📋 {len(guides)} guides sans sections_ar trouvés')
        count = 0
        for g in guides[:args.limit]:
            ok = process_guide(g['slug'], args.dry_run)
            count += 1
            if count < min(args.limit, len(guides)) and not args.dry_run:
                time.sleep(args.delay)
        print(f'\n✅ Traités : {count}/{len(guides)}')
    else:
        print('Usage: --slug <slug> | --all [--dry-run] [--limit N]')
        guides = list_missing()
        print(f'\n{len(guides)} guides sans sections_ar :')
        for g in guides:
            has_fr = '✓FR' if g['sections_fr'] else '✗FR'
            print(f'  {has_fr}  {g["slug"]}')
